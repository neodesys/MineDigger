...
namespace sys
{
    ...
    Renderer* Renderer::createRenderer(const char* wndName, int width, int height)
    {
        ...
        SDL_Window* pSDLWnd =
            SDL_CreateWindow(wndName, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, 0);
        if (!pSDLWnd)
        {
        ...
        {
            s_log.warning(
                "Cannot create hardware accelerated SDL2 renderer (%s), falling back to software rendering...",
                SDL_GetError());
    ...
    void Renderer::drawTexture(const Texture& tex, const Rect* pSrcRect, const Rect* pDestRect)
    {
        ...
        if (SDL_RenderCopy(m_pSDLRdr, tex.m_pSDLTex, reinterpret_cast<const SDL_Rect*>(pSrcRect),
                           reinterpret_cast<const SDL_Rect*>(pDestRect)))
