...
namespace app
{
    ...
    {
        if (pAnimPath && uAnimPathLength && pAnimPath[uAnimPathLength - 1].uTimestamp)
        {
    ...
    void PathSprite::update(const sys::FrameInfo& frame)
    {
        ...
        else
        {
            ...
            if ((pPrevKey->uTimestamp >= pNextKey->uTimestamp) || (uAnimOffset < pPrevKey->uTimestamp))
            {
            ...
            float f =
                static_cast<float>(uAnimOffset - pPrevKey->uTimestamp) / (pNextKey->uTimestamp - pPrevKey->uTimestamp);
