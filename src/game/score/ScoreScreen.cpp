...
namespace game
{
	namespace score
	{
		ScoreScreen::ScoreScreen(MineDigger& game) :
			m_game(game),
			m_config(game.getConfig().scoreScreenConfig),
			m_backButton(*this) {}
		...
		void ScoreScreen::onGameScreenStart(sys::AudioMixer& mixer)
		{
			...
			m_backButton.setOverlay(m_pScoreTex, &m_config.backButtonOverlayClip, m_config.uBackButtonOverlayImgCount, m_config.uBackButtonOverlayImgStride);
		...
		void ScoreScreen::update(const sys::FrameInfo& frame)
		{
			...
			case AnimState::BACK_BUTTON:
				if (m_pScoreMusic &&
					(m_pScoreMusic->getStatus() != sys::AudioStatus::PLAYING) &&
