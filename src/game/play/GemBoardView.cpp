	...
	namespace play
	{
		...
		void GemBoardView::onMouseMove(const sys::Vec2& pos, bool bDragging)
		{
			if (m_pDraggedSprite)
			{
				...
				if ((row < GemBoardModel::NB_ROWS - 1) && (offset.distance({0, -m_config.slotHeight}) < 0.5f * m_config.slotHeight))
				{
					//Try up
				...
				else if ((col < GemBoardModel::NB_COLS - 1) && (offset.distance({m_config.slotWidth, 0}) < 0.5f * m_config.slotWidth))
				{
					//Try right
		...
		{
			if ((row >= 0) && (row < GemBoardModel::NB_ROWS) &&
