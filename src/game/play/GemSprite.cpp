	...
	namespace play
	{
		...
		void GemSprite::initGem(GemType type, int row, int col)
		{
				...
				(col >= 0) && (col < GemBoardModel::NB_COLS))
			{
				...
				m_pos.y = (config.startLine - 1) - (config.spriteConfig.initRowSpacingFactor * row + 0.5f) * config.slotHeight;
		...
		{
			if (m_pView &&
				((m_state == State::IN_PLACE) || (m_state == State::SPRING_ATTACHED)) &&
				((row != m_modelRow) || (col != m_modelCol)) &&
				((m_swapModelRow < 0) || (m_swapModelCol < 0)) &&
				(row >= 0) && (row < GemBoardModel::NB_ROWS) &&
				(col >= 0) && (col < GemBoardModel::NB_COLS))
			{
				unselect();
