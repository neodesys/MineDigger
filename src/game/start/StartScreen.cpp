...
namespace game
{
	namespace start
	{
		StartScreen::StartScreen(MineDigger& game) :
			m_game(game),
			m_config(game.getConfig().startScreenConfig),
			m_startBoard(*this)
		{
		...
		void StartScreen::onGameScreenStart(sys::AudioMixer& mixer)
		{
			...
			m_startBoard.setOverlay(m_pStartBoardTex, &m_config.startBoardOverlayClip, m_config.uStartBoardOverlayImgCount, m_config.uStartBoardOverlayImgStride);
